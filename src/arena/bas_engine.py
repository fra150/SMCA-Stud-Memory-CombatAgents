...
class BASEngine:
    ...
    def _build_arena(self, agents: List[SegmentAgent]) -> Arena:
        ...
        if not agents:
            # Arena vuota
            red_agent_to_pass = self.red_agent if self.enable_red_agent else None
            self.arena = Arena([], self.judge, god_protocol=self.god, red_agent=red_agent_to_pass, studsar_manager=self.studsar)
            return self.arena
        ...
        # Crea arena con Red Agent per Exhaustive Consensus Search se abilitato
        red_agent_to_pass = self.red_agent if self.enable_red_agent else None
        self.arena = Arena(
            combat_agents, 
            self.judge, 
    ...
    def query(self, question: str, max_rounds: int = 3,
        ...
        # La resilienza è ora calcolata esaustivamente durante l'arena combat (Exhaustive Consensus Search).
        ...
        result = BASResult(
            ...
            champion_resilience_score=0.0,  # Resilience now computed exhaustively inside Arena
