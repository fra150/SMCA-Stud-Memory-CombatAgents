...
Benchmark che testa la capacità di BAS di:
...
class LOCOMOBenchmark:
    ...
    def run_test(self, test_type: str, length: int, questions: Dict[str, str], 
        ...
        stats = self.bas.ingest_document(document, f"LOCOMO_{test_type}_{length}", segment_length=100)
        ingest_time = time.time() - start_ingest
        print(f"  ✓ Created {stats.get('agents_created', 0)} agents in {ingest_time:.2f}s\n")
        ...
        for q_key, question in questions.items():
            ...
            actual_answer = response.final_answer
            confidence = response.judge_confidence
            memory_coherence = response.memory_coherence_score
            participating_agents = response.active_agents
            ...
            result = BenchmarkResult(
                ...
                agents_used=participating_agents,
    ...
    def print_summary(self):
        ...
        output_file = os.path.join(os.path.dirname(__file__), "locomo_benchmark_results.json")
